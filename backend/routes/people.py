...
def add_person():
    ...
    if any(str(p.get("code")) == code for p in data[category]["active"]):
        return jsonify({"error": "رقم الأقدمية مستخدم بالفعل على القوة."}), 409
...
def edit_person(person_id):
    ...
    if "code" in payload:
        code = str(payload["code"]).strip()
        if not code:
            return jsonify({"error": "رقم الأقدمية مطلوب."}), 400
    ...
    if bucket == "archive" and "leave_date" in payload:
        ...
        if payload["leave_date"] < person.get("join_date", ""):
            return jsonify({"error": "تاريخ الخروج لا يمكن أن يسبق تاريخ الانضمام."}), 400
...
def remove_person(person_id):
    ...
    if not found or bucket != "active":
        return jsonify({"error": "الشخص غير موجود على القوة."}), 404
...
def restore_person(person_id):
    ...
    if not found or bucket != "archive":
        return jsonify({"error": "السجل غير موجود في الأرشيف."}), 404

    code = str(found.get("code", ""))
    if any(str(p.get("code")) == code for p in data[category]["active"]):
        return jsonify({"error": "يوجد شخص على القوة بنفس رقم الأقدمية."}), 409
